 ...
 */
export function selectLocalSteersForQueue(chips: PendingSteer[]): TPendingSteer[] {
  return chips
    .filter((steer) => steer.status === 'pending' || steer.status === 'failed')
    .map((steer) => ({
  ...
  const convertLocalSteersToQueued = useRecoilCallback(
    ({ snapshot }) =>
      (conversationId: string, options?: { claimParked?: boolean }) => {
        ...
        const settled = selectLocalSteersForQueue(chips);
        if (settled.length > 0) {
