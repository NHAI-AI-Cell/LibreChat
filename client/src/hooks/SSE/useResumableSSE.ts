...
export function selectLocalSteersForQueue(
  ...
  return chips
    .filter((steer) => allowed.has(steer.status))
    .map((steer) => ({
...
export default function useResumableSSE(
  ...
  const convertLocalSteersToQueued = useRecoilCallback(
    ({ snapshot }) =>
      (
        conversationId: string,
        options?: { claimParked?: boolean; statuses?: readonly PendingSteer['status'][] },
      ) => {
        const chips = snapshot.getLoadable(store.pendingSteersByConvoId(conversationId)).getValue();
        const settled = selectLocalSteersForQueue(chips, options?.statuses);
        if (settled.length > 0) {
          convertSteersToQueued(conversationId, settled, { claimParked: options?.claimParked });
        }
      },
